...
The drone finds targets by looking for a red circle in its camera picture. A red marker can look
...
few red pixels does not automatically count as finding a target.
...
pixels match the configured colour?” It does not mean every highlighted patch is a detected target.
