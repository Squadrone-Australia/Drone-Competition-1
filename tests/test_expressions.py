...
def test_number_literal_and_bare_number_are_the_same():
    p = prog([{"id": "a", "op": "set_var", "name": "x", "value": 7},
              {"id": "b", "op": "set_var", "name": "y",
               "value": {"kind": "number", "value": 7}}])
    assert p.blocks[0].value == p.blocks[1].value


def test_bare_numbers_are_lifted_inside_nested_values():
    node = prog([{"id": "a", "op": "set_var", "name": "x",
                  "value": {"kind": "binop", "op": "+", "left": 2, "right": 3}}]).blocks[0].value
    assert value_of(node)[0] == 5


def test_value_round_trips_through_json():
    src = {"version": 2, "blocks": [
        {"id": "a", "op": "move", "dir": "forward",
         "cm": {"kind": "binop", "op": "*", "left": {"kind": "var", "name": "s"}, "right": 10}}]}
...
def test_var_read_and_unop_and_binop_evaluate():
    node = prog([{"id": "a", "op": "set_var", "name": "x", "value": {
        "kind": "binop", "op": "-",
        "left": {"kind": "unop", "op": "abs", "operand": -4},
        "right": {"kind": "unop", "op": "neg", "operand": {"kind": "var", "name": "s"}}}}
    ]).blocks[0].value
    assert value_of(node, s=6.0)[0] == 10          # abs(-4) - (-6)


@pytest.mark.parametrize("op,left,right,want", [
    ("+", 3, 4, 7), ("-", 3, 4, -1), ("*", 3, 4, 12), ("/", 3, 4, 0.75),
    ("<", 3, 4, True), (">", 3, 4, False), ("<=", 4, 4, True), (">=", 3, 4, False),
    ("==", 4, 4, True), ("!=", 4, 4, False),
    ("and", 1, 0, False), ("and", 1, 2, True), ("or", 0, 0, False), ("or", 0, 3, True),
])
...
def test_nested_arithmetic_and_comparison():
    # (2 + 3) * 4 > 19
    node = prog([{"id": "a", "op": "set_var", "name": "x", "value": {
        "kind": "binop", "op": ">", "right": 19, "left": {
            "kind": "binop", "op": "*", "right": 4,
            "left": {"kind": "binop", "op": "+", "left": 2, "right": 3}}}}]).blocks[0].value
    assert value_of(node)[0] is True


def test_not_uses_truthiness():
    node = prog([{"id": "a", "op": "set_var", "name": "x",
                  "value": {"kind": "unop", "op": "not", "operand": 0}}]).blocks[0].value
    assert value_of(node)[0] is True
...
def sensor(name, det=None):
    node = prog([{"id": "a", "op": "set_var", "name": "x",
                  "value": {"kind": "sensor", "sensor": name}}]).blocks[0].value
...
def test_sensors_without_a_target_use_the_documented_sentinels():
    ...
    assert sensor("target_distance_cm") == 9999.0     # "far away", so naive searches keep turning
...
async def test_distance_sentinel_keeps_a_naive_search_loop_running():
    drone, _, _ = await run([
        {"id": "a", "op": "repeat_until",
         "cond": {"kind": "binop", "op": "<",
                  "left": {"kind": "sensor", "sensor": "target_distance_cm"}, "right": 120},
...
async def test_found_count_and_battery_sensors():
    ...
    it.found_count = 3
    node = prog([{"id": "a", "op": "set_var", "name": "x",
                  "value": {"kind": "sensor", "sensor": "found_count"}}]).blocks[0].value
    assert it._eval(node) == 3
    node = prog([{"id": "a", "op": "set_var", "name": "x",
                  "value": {"kind": "sensor", "sensor": "battery"}}]).blocks[0].value
    assert it._eval(node) == 100.0                     # MockDrone


def test_unknown_sensor_rejected_at_parse():
    with pytest.raises(ValidationError):
        prog([{"id": "a", "op": "set_var", "name": "x",
...
async def test_set_var_then_read_it():
    _, _, it = await run([
        ...
        {"id": "b", "op": "set_var", "name": "steps",
         "value": {"kind": "binop", "op": "*", "left": {"kind": "var", "name": "steps"},
                   "right": 2}},
    ])
...
async def test_variables_drive_a_move():
    drone, events, _ = await run([
        ...
        {"id": "b", "op": "move", "dir": "forward", "cm": {"kind": "var", "name": "d"}},
    ])
...
async def test_unset_variable_reads_zero_with_a_warning():
    drone, events, _ = await run([
        {"id": "a", "op": "move", "dir": "forward",
         "cm": {"kind": "binop", "op": "+", "left": {"kind": "var", "name": "nope"},
                "right": 30}}])
...
async def test_bools_are_themselves():
    drone, _, _ = await run([
        {"id": "a", "op": "if", "cond": {"kind": "sensor", "sensor": "target_visible"},
         "body": [{"id": "b", "op": "flip", "dir": "left"}]}], det=seen())
...
async def test_repeat_n_clamps_and_zero_skips_the_body():
    drone, events, _ = await run([
        {"id": "z", "op": "repeat_n", "n": computed(0),
         "body": [{"id": "b", "op": "flip", "dir": "left"}]},
        {"id": "hi", "op": "repeat_n", "n": computed(80),
...
async def test_wait_clamps_both_ways():
    ...
    await it.run(prog([{"id": "lo", "op": "wait", "seconds": computed(-5)},
...
async def test_wait_actually_waits_and_is_interruptible():
    import asyncio
    it, events = make()
    task = asyncio.create_task(it.run(prog([{"id": "a", "op": "wait", "seconds": 10},
...
async def test_while_runs_while_true_and_repeat_until_runs_until_true():
    ...
    drone, events, _ = await run([
        {"id": "u", "op": "repeat_until", "cond": vis,
         "body": [{"id": "x", "op": "flip", "dir": "left"}]},
        {"id": "w", "op": "while", "cond": {"kind": "unop", "op": "not", "operand": vis},
...
async def test_while_terminates_on_a_counter():
    drone, events, it = await run([
        ...
        {"id": "w", "op": "while",
         "cond": {"kind": "binop", "op": "<", "left": {"kind": "var", "name": "i"}, "right": 3},
         "body": [
             {"id": "f", "op": "flip", "dir": "left"},
             {"id": "b", "op": "set_var", "name": "i",
              "value": {"kind": "binop", "op": "+", "left": {"kind": "var", "name": "i"},
                        "right": 1}}]},
    ])
...
V1_CONDITIONS = [
    ...
    ({"sensor": "found_count_gte", "value": 2},
     {"kind": "binop", "op": ">=", "left": {"kind": "sensor", "sensor": "found_count"},
...
def test_v1_conditions_upgrade_to_the_documented_value_nodes(v1, v2):
    ...
    assert p.version == 2
    assert p.blocks[0].cond == Program.model_validate(
        {"version": 2, "blocks": [{"id": "a", "op": "if", "cond": v2}]}).blocks[0].cond


def test_v1_conditions_upgrade_inside_nested_bodies():
    p = prog([{"id": "a", "op": "repeat_until", "cond": {"sensor": "marker_visible"},
               "body": [{"id": "b", "op": "if", "cond": {"sensor": "marker_position_center"},
                         "else_body": [{"id": "c", "op": "if",
...
async def test_v1_found_count_gte_still_gates_on_the_count():
    drone, _, _ = await run([
        {"id": "a", "op": "if", "cond": {"sensor": "found_count_gte", "value": 1},
         "body": [{"id": "b", "op": "flip", "dir": "left"}],
         "else_body": [{"id": "c", "op": "mark_found"}]},
        {"id": "d", "op": "if", "cond": {"sensor": "found_count_gte", "value": 1},
         "body": [{"id": "e", "op": "flip", "dir": "forward"}]},
    ], version=1)
    assert drone.log == [("flip", "back"), ("flip", "forward")]   # mark_found, then the gate opens
