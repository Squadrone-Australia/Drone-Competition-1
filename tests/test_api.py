...
def sim_session(world=None, heading=0.0, **kw):
    ...
    return Session(
        drone=adapter, get_detection=lambda: tracker.update(adapter.get_frame()), **kw
    ), adapter
...
def test_every_movement_maps_to_the_right_adapter_call():
    ...
    d = Drone()
    d._s, d._d = session, adapter  # bind explicitly, no global
    d.takeoff()
    d.forward(50)
    d.back(60)
    d.left(70)
...
async def test_a_loop_with_no_drone_calls_is_abandoned_and_left_powerless():
    ...
    _, (reason, detail) = await asyncio.gather(
        stop_when_flying(), run.run(student_code)
    )
