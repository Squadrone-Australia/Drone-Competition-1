...
``DroneAdapter``, reads the same OpenCV ``Detection``, and closes on a target
...
A whole mission::
    ...
    drone = Drone()
    drone.takeoff()
    while not drone.sees_target():      # spin until a target comes into view
        drone.turn_right(20)
...
class TargetView:
    """A target in the units a student thinks in, with no pixels exposed."""
    ...
    def __str__(self):
        return (
            f"target {self.distance_cm:.0f} cm away, "
...
class Drone:
    ...
    def sees_target(self) -> bool:
        """True if a red target is visible right now."""
        self._check()
        return self._s.get_detection().found

    def target(self) -> TargetView | None:
        """The target the drone is locked onto, or None if it cannot see one.
    ...
    def targets(self) -> list[TargetView]:
        """Every target in view, nearest first."""
        self._check()
        return [_view(t) for t in self._s.get_detection().targets]

    def distance_cm(self) -> float | None:
        """How far away the target is in centimetres, or None if none is visible."""
        t = self.target()
        return t.distance_cm if t else None

    def bearing_deg(self) -> float | None:
        """Which way to turn to face the target: + is right, - is left.

        None if no target is visible.
        """
        t = self.target()
        return t.bearing_deg if t else None

    def elevation_deg(self) -> float | None:
        """How far above (+) or below (-) the camera the target sits, in degrees."""
        t = self.target()
    ...
    def approach_target(self, stop_distance_cm: float | None = None) -> bool:
        """Choose the nearest target, fly to it, and stop a safe distance short.

        Returns True once it is in position, False if the target was lost or it
        ...
        for _ in range(cfg.approach_max_steps):
            ...
            if not det.found:
                ...
                # bursts, and a burst is not a target that has gone away.
                now = time.monotonic()
    ...
    def found_count(self) -> int:
        """How many targets you have marked so far this run."""
